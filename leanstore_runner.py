...
if __name__ == "__main__":
    ...
    if output_file is None or len(output_file) < 1:
        print("Usage: python3 leanstore_runner.py <output file names> <tpcc_directory> ")
        sys.exit(1)
    ...
    command = f"./tpcc --tpcc_warehouse_count=1 --notpcc_warehouse_affinity --worker_threads=4 --pp_threads=16 --free_pct=1 --contention_split --xmerge --warmup_for_seconds=1 --run_for_seconds=2 --isolation_level=si --bookkeeper_jar_directories=../../bookkeeper-wal/target:../../bookkeeper-wal/target/maven-dependencies  --wal_pwrite=true --profile_latency=true --csv_path={output_file} --wal_variant=0"
